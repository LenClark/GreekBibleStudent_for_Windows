...
namespace GreekBibleStudent
{
    public class classNTText
    {
        ...
        {
            int idx, bookNo, ntBookCount = 0, maxCategory = 0, minCategory = 100;
             ...
             *                                                        *
             *--------------------------------------------------------*/
            progressForm.Invoke(new performProgressAdvance(updateProgress), "Loading the names of NT books", "", false);
            titlesFileName = globalVars.NtTitlesFile;
            srSource = new StreamReader(titlesFileName);
            fileBuffer = srSource.ReadLine();
            bookNo = globalVars.NoOfLXXBooks;
            while (fileBuffer != null)
            {
                ...
                if (fields.Length > 3)
                {
                    ...
                    currentBook.FileName = fields[2];
                    currentBook.Category = Convert.ToInt32(fields[3]);
                    currentBook.IsNT = true;
                    globalVars.getComboBoxItem(0).Invoke(new performComboBoxUpdate(addComboItem), globalVars.getComboBoxItem(0), currentBook.BookName);
                    bookNo++;
                    ntBookCount++;
                    if( currentBook.Category > maxCategory) maxCategory = currentBook.Category;
                    if (currentBook.Category < minCategory) minCategory = currentBook.Category;
                }
                fileBuffer = srSource.ReadLine();
            }
            srSource.Close();
            globalVars.NoOfNTBooks = ntBookCount;
            globalVars.NoOfAllBooks = bookNo;
            globalVars.NoOfBookGroups = maxCategory;
            ...
            noInEachCategory = new int[maxCategory - minCategory + 1];
            for (idx = 0; idx < maxCategory - minCategory + 1; idx++) noInEachCategory[idx] = 0;
            for (idx = globalVars.NoOfLXXBooks; idx < globalVars.NoOfAllBooks; idx++)
            {
                globalVars.BookList.TryGetValue(idx, out currentBook);
                globalVars.DifferentiatedList[currentBook.Category - 1, noInEachCategory[currentBook.Category - minCategory]] = currentBook;
                noInEachCategory[currentBook.Category - minCategory]++;
            }
            ...
            {
                globalVars.BookList.TryGetValue(idx, out currentBook);
                progressForm.Invoke(new performProgressAdvance(updateProgress), "Loading source text for the New Testament", currentBook.BookName, true);
                currentChapter = null;
                currentVerse = null;
                prevChapRef = "?";
                sourceFileName = globalVars.NtTextFolder + @"\" + currentBook.FileName;
            ...
            globalVars.getComboBoxItem(0).Invoke(new performComboBoxSelection(selectComboItem), globalVars.getComboBoxItem(0), 0);
            globalVars.getLabel(2).Invoke(new performMainFormLabelChange(changeLabel), globalVars.getLabel(2), "NT Load Complete");
        }
        ...
        public void displayChapter(int bookIdx, String chapIdx, bool isSameBook)
        {
            ...
            greekFont = globalVars.configureFont(globalVars.getDefinedFontNameByIndex(0, 2), globalVars.getDefinedStyleByIndex(0, 2), globalVars.getTextSize(0, 2));
            ...
            engTextColour = globalVars.getColourSetting(0, 1);
            gkTextColour = globalVars.getColourSetting(0, 2);
            ...
            if ( ! isSameBook) cbBook.SelectedIndex = bookIdx - globalVars.NoOfLXXBooks;
            // Get the specified chapter from the current book -
            currentChapter = currentBook.getChapterByChapterRef(chapIdx);
            if (!isSameBook)
            {
                // Modify the chapter combo box *and* display the new chapter - not needed if same Book
...
//            globalVars.IsOnHold = false;
            ...
            if (globalVars.getTabControl(0).SelectedTab == globalVars.getTabCtrlPage(0))
            {
                ...
                globalVars.MasterForm.Text = "Greek Bible Student - New Testament: " + displayString;
        ...
        public void backOrForwardOneChapter(int forwardBack)
        {
            ...
            cbBooks = globalVars.getComboBoxItem(0);
            cbChapters = globalVars.getComboBoxItem(1);
            actualIdx = cbBooks.SelectedIndex + globalVars.NoOfLXXBooks;
        ...
        public void processSelectedHistory()
        {
            ...
            cbHistory = (ComboBox)globalVars.getComboBoxItem(6);
            if (cbHistory.Items.Count == 0) return;
            historyEntry = cbHistory.SelectedItem.ToString();
            if (String.Compare(globalVars.LastNTHistoryEntry, historyEntry) == 0) return;
            ...
            if (bookIdx > -1)
            {
                globalVars.LastNTHistoryEntry = historyEntry;
        ...
        private void addEntryToHistory(String displayString)
        {
            ...
            cbHistory = globalVars.getComboBoxItem(6);
        ...
        public void recordMouseEffects(MouseEventArgs e)
        {
            ...
            cbChapter = globalVars.getComboBoxItem(1);
            cbVerse = globalVars.getComboBoxItem(2);
            currentRtxt = globalVars.getRichtextItem(0);
            ...
            globalVars.LastSelectedNTVerse = currentVerseText;
            globalVars.LastSelectedNTWord = selectedWord;
            globalVars.SelectedNTWordSequence = wordSeq;
            globalVars.CurrentlySelectedNTWord = currentWord;
        }
        ...
        public void Analysis()
        {
            ...
            if (globalVars.LastSelectedNTWord.Length == 0)
            {
            ...
            currentWord = currentVerse.getWordBySeqNo(globalVars.SelectedNTWordSequence);
            ...
            parseString = lexicon.parseNTGrammar(currentWord.CatString, currentWord.ParseString);
